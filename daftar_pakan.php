...
$queryPakan = "SELECT * FROM mbek_pakan WHERE id_pakan LIKE '%$search_keyword%' ORDER BY date_record DESC LIMIT $offset, $records_per_page";
...
<div style="display: flex; justify-content: center; margin: 20px;">
            ...
            <table class="w3-table-all w3-centered" border="1" style="border-collapse: collapse; width: 100%;">
                ...
                <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                    <tr class="pakan-row">
                        ...
                        <td style="font-size: 15px;"><?php echo htmlspecialchars($row['berat']); ?></td>
                        <td style="font-size: 15px;"><?php echo htmlspecialchars($row['harga_pakan']); ?></td>
  ...
  <style>
    ...
    modalMessage.textContent = "Apakah Anda yakin ingin menghapus hewan '" + id_pakan + "'?";

    // Simpan nama hewan yang akan dihapus di button 'Hapus'
    var confirmButton = modal.querySelector('.w3-button.w3-red');
    confirmButton.onclick = function () {
        window.location.href = "hapus_hewan.php?id_pakan=" + encodeURIComponent(id_pakan);
    };
}
