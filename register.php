...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($password !== $password_confirm) {
        ...
        $check_query = $conn->prepare("SELECT username FROM mbek_pengguna WHERE username = ?");
        ...
        if ($check_query->num_rows > 0) {
            ...
            $query = $conn->prepare("INSERT INTO mbek_pengguna (username, password, nama) VALUES (?, ?, ?)");
...
<body class="w3-green">
    ...
    <form class="w3-container w3-card-4 w3-light-grey w3-padding-16 w3-margin" action="" method="post"
        style="border-radius: 10px;" onsubmit="return trimInputs()">
        ...
        <div class="w3-input-container" style="position: relative; margin-bottom: 30px;">
            ...
            <input type="password" id="password_confirm" class="w3-input w3-border-bottom w3-light-grey password-field"
                name="password_confirm" required>
            <!-- Eye icon inside the input field -->
            <i id="togglePasswordConfirm" class="fa fa-eye"
                style="position: absolute; right: 10px; top: 35px; cursor: pointer; z-index: 1;"></i>
        </div>


        <button type="submit" class="w3-input w3-button w3-round-large w3-green w3-margin-top">Register</button>
        <a href="index.php" class="w3-input w3-button w3-round-large w3-grey w3-margin-top">Kembali</a>
    </form>
